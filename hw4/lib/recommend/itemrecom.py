...
# Author: Sergei Usovik <usovik@bmstu.ru>
...
class ItemBasedRecommend:
    ...
    def train(self, df_train, top_N=None, user_column_name="user", item_column_name="item",
        ...
        self.df_similarity = df_similarity_N.persist()
        self.df_norm = df_norm.persist()
    ...
    def predict(self, df_test, user_column_name="user", item_column_name="item",
        ...
        df_test = df_test.select(clmn_names)
        
        # (user, (item, rating))
        ratings_RDD = self.df_train.rdd.map(lambda r: (r["user"], (r["item"], r["rating"])))
        
        # ((item, item), rating)
        similarity_RDD = (
           self.df_similarity.rdd.flatMap(lambda s: [
                ((s["p1"], s["p2"]), s["sim"]),
                ((s["p2"], s["p1"]), s["sim"])
            ])
        )
        
        # преобразуем similarity к ключу j (с которым сравниваем)
        # [(item, (item, rating))]
        sim_by_j_RDD = similarity_RDD.map(lambda x: (x[0][1], (x[0][0], x[1])))
        
        # нужно для каждой оценки (u, j, rating) найти все i, похожие на j:
        mapped_RDD = (
            ratings_RDD
              .flatMap(lambda x: [ (x[1][0], (x[0], x[1][1])) ])  # (item, (user, rating))
              .join(sim_by_j_RDD)                                 # по общему item -> item, ((item,rating), (user,rating))
              .map(lambda x: (
                    (x[1][0][0], x[1][1][0]),                    # key=(user, item)
                    (x[1][0][1] * x[1][1][1], abs(x[1][1][1]))   # value=(wsum, |sim|)
              ))
        )
        
        agg_RDD = mapped_RDD.reduceByKey(
            lambda a, b: (a[0] + b[0], a[1] + b[1])
        )
        
        predicted_RDD = agg_RDD.mapValues(lambda v: v[0]/v[1] if v[1] != 0 else 0.0)

        predicted_DF = predicted_RDD.map(lambda x: (x[0][0], x[0][1], x[1])) \
                                    .toDF(["user", "item", "rating_pred"])
    ...
    def predict_single(self, user_id, item_id):
        ...
        df_sim_filtered = self.df_similarity\
            .filter((F.col("p1") == movieId) | (F.col('p2') == movieId)).sort('p2', ascending=True)

        df_predict_example = self.df_train\
            .where(F.col('user') == user)\
            .join(self.df_similarity, ((self.df_train.item == df_sim_filtered.p1) & (df_sim_filtered.p2 == movieId)) | 
                                      ((self.df_train.item == df_sim_filtered.p2) & (df_sim_filtered.p1 == movieId))) \
            .sort('sim', ascending=False) \
            .limit(self.top_N_similarities)

        df_predict_example2 = self.df_train.where(F.col('user') == user).join(self.df_norm, self.df_norm.p1 == self.df_train.item)
        ...
        return w_sum / w_norms if w_sum and w_norms else None
